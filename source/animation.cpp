...
Animation::Animation(const Uint8 speed, const LoopType loopType, const Uint16 loops) {
	_speed = speed;
	_frameTimer = speed;
	_currentFrame = 0;
...
SDL_Surface* Animation::getCurrentSurface() const {
	return _frames[_currentFrame].surface;
}
...
const Uint8 Animation::getSpeed() const {
	return _speed;
}
...
const Uint16 Animation::getTimeToNextFrame() const {
	return _frameTimer;
}

// Returns the total number of animation frames
const Uint16 Animation::getFrameCount() const {
	return _frames.size();
}

// Sets the number of VBLs that pass before the next frame is shown
void Animation::setSpeed(const Uint8 speed) {
	_speed = speed;
}
...
void Animation::addFrame(BitmapServer::BitmapType type, const Uint8 delay) {

	// Create new animation frame
	AnimFrame newFrame;
	newFrame.surface = BitmapServer::getBitmap(type);
...
// Jump to a new frame
void Animation::goToFrame(Uint16 frame) {
	_currentFrame = frame;
	_frameTimer = _speed + _frames[_currentFrame].delay;
}
